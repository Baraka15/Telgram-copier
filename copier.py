...
def parse_trade(text):
    ...
    if not side or len(prices) < 3:
        return None

    entry = float(prices[0])
    sl = float(prices[1])
    tp = float(prices[2])  # First TP only
    ...
    while True:
        ...
        try:
            ...
            if rr_data["rr"] < MIN_RR:
                logger.info(f"🚫 Low RR {message.id}")
                continue
            ...
            save_hash(msg_hash)
            logger.info(f"✅ Worker {wid} sent {message.id}")
