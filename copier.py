...
async def worker(client, wid):
    ...
    while True:
        ...
        try:
            text = message.raw_text or ""
            logger.info(f"🧪 Worker-{wid} processing msg {message.id}")

            if not passes_filter(text):
                logger.info(f"🚫 Worker-{wid} filtered msg {message.id}")
                continue

            msg_hash = build_hash(message)

            if is_duplicate(msg_hash):
                logger.info(f"🔁 Worker-{wid} duplicate msg {message.id}")
                continue
            ...
            if message.media:
                ...
                if text.strip():
                    ...
                    logger.info(f"📝 Worker-{wid} sent caption {message.id}")
