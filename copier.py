...
# Restart between 5h 30m and 5h 45m to beat GitHub's 6h limit
MAX_RUNTIME = random.uniform(330, 345) * 60 
STARTUP_GRACE = 15

# MOVED TO state_data/ FOR GITHUB CACHING
...
# =========================
# GITHUB RESTART (FIXED)
# =========================
def trigger_github_restart():
    if not REPO or not GH_TOKEN:
        logger.error("❌ Restart aborted: Missing GITHUB_REPOSITORY or GH_TOKEN")
        return False

    # Trigger via workflow_dispatch for the copier.yml file
    ...
    payload = {"ref": "main"} # Or your default branch name

    try:
        res = requests.post(url, headers=headers, json=payload, timeout=15)
        if res.status_code == 204:
            logger.info("🚀 GitHub restart signal sent successfully")
            return True
        else:
...
async def runtime_guard(client):
    ...
    logger.warning("⏳ Approaching 6h limit. Triggering restart...")
...
async def worker(client, wid):
    while not shutdown_event.is_set():
        try:
            ...
            # Skip old messages
            if message.date and message.date < BOOT_TIME - timedelta(seconds=30):
                queue.task_done()
                continue
...
async def start_client():
    ...
    async def handler(event):
        try:
            priority = 0 if detect_signal(event.message.raw_text) else 1
            queue.put_nowait((priority, event.message))
        except:
            pass

    logger.info("⚡ Bot is now 24/7 Active")
