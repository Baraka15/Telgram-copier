...
SOURCE_CHATS: List[Dict] = config.get(
    "sources",
    [
        {
...
TARGET_CHATS: List[Dict] = config.get(
    "targets",
    [
        {
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.handlers.RotatingFileHandler(
            "copier.log", maxBytes=10**6, backupCount=5
        ),
    ],
)
...
if GLOBAL_SETTINGS["metrics_enabled"]:
    forward_counter = prom.Counter(
        "forwards_total", "Total forwarded messages", ["type", "source"]
    )
...
def is_duplicate(msg_hash: str, source_id: int) -> bool:
    cutoff = datetime.utcnow() - timedelta(
        minutes=GLOBAL_SETTINGS["dedup_window_minutes"]
    )
...
def record_hash(msg_hash: str, source_id: int, message_id: int, metadata: Dict):
    ...
    cursor.execute(
        ...
        "(hash, source_id, message_id, timestamp, metadata) "
        "VALUES (?, ?, ?, ?, ?)",
...
async def cleanup_db():
    ...
    db_conn.commit()
    logger.info("🧹 DB cleanup: Removed old records")
...
def transform_message(text: str, transform: Dict) -> str:
    ...
    suffix = transform.get("suffix", "").format(
...
def detect_media_type(message: Message) -> Optional[str]:
    ...
    if isinstance(media, MessageMediaDocument):
        return "document"
    if isinstance(media, MessageMediaVideo):
        return "video"
    if isinstance(media, MessageMediaAudio):
        return "audio"
    ...
    for target in TARGET_CHATS:
        ...
        async def perform_forward():
            if message.media and rules["allow_media"]:
                ...
                transformed_text = transform_message(
                    message.text, source_config.get("transform", {})
                )
        ...
        try:
            ...
            await send_webhook(
                {"error": str(e), "type": "access", "source": source_id}
            )
        except RPCError as e:
            error_counter.inc(labels={"type": "rpc"})
            logger.error(f"⚠ RPC error: {e}")
            await send_webhook(
                {"error": str(e), "type": "rpc", "source": source_id}
            )
        except Exception as e:
            error_counter.inc(labels={"type": "unknown"})
            logger.error(f"⚠ Unknown error: {e}")
            await send_webhook(
                {"error": str(e), "type": "unknown", "source": source_id}
            )
...
async def main():
    ...
    async def handler(event):
        logger.info(
            f"📩 New message detected | Chat: {event.chat_id} | ID: {event.message.id}"
        )
        source_config = next(
            (s for s in SOURCE_CHATS if s["chat_id"] == event.chat_id), None
        )
        if source_config:
    ...
    scheduler.add_job(
        cleanup_db, CronTrigger.from_crontab(GLOBAL_SETTINGS["cleanup_cron"])
    )
    scheduler.start()
