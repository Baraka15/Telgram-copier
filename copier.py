...
def save_hash(msg_hash):
    ...
    if len(hashes) > MAX_HASHES:
        excess = len(hashes) - MAX_HASHES
        for _ in range(excess):
            old = hashes.pop(0)
            KNOWN_HASHES.discard(old)
...
def build_hash(message):
    ...
    if message.media:
        try:
            media_id = str(message.media.document.id) if message.media.document else ""
        except:
...
async def worker(client, wid):
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"Worker-{wid} crash: {e}")
