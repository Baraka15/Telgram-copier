...
SOURCE_CHATS = [int(x.strip()) for x in require_env("SOURCE_CHATS").split(",")]
...
duplicate_cache = deque(maxlen=DUP_LIMIT)
...
async def forward_worker():
    ...
    while True:
        ...
        try:
            ...
            duplicate_cache.append(unique_key)
            send_times.append(time.time())

            risk = adjust_delay()

            logger.info(
                f"Forwarded | Q={message_queue.qsize()} | Risk={round(risk,3)} | Delay={round(adaptive_delay,2)}"
            )
